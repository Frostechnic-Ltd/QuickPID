...
QuickPID::QuickPID(float* Input, float* Output, float* Setpoint,
  ...
  QuickPID::SetTunings(Kp, Ki, Kd, POn, DOn, 100000);
...
bool QuickPID::Compute() {
  ...
  if (mode == TIMER || timeChange >= sampleTimeUs) {
    ...
    // Serial.printf("%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f\n", *myInput, error, *myOutput, outputSum, pmTerm, peTerm, iTerm, dmTerm, deTerm, dInput, dError, kpm, kpe, ki, kdm, kde);
    ...
    peTerm = kp * error;
    (ki != 0) ? iTerm =  iTerm + ((ki*kp)*(error+lastError))/2 : iTerm = 0;
    // deTerm = (kde*kp) * dError;
    float SampleTimeSec = (float)sampleTimeUs / 1000000;
    tf = 1;
    deTerm = (2* kd *dInput + (2 * tf - SampleTimeSec) * deTerm) / (2* tf + SampleTimeSec);
    ...
    //compute output as per PID_v1
    //outputSum += iTerm;
    outputSum = constrain(outputSum, outMin, outMax);                                                             // add integral amount
    //*myOutput = constrain(outputSum +iTerm + peTerm - deTerm, outMin, outMax);  // totalize, clamp, drive output
    *myOutput = constrain(iTerm + peTerm - deTerm, outMin, outMax);  // totalize, clamp, drive output
    Serial.printf("%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f\n", error, *myOutput, peTerm, iTerm,deTerm, dError, kpm, kpe, kp, ki, kdm, kde);
    lastError = error;
    lastInput = input;
    lastTime = now;
    return true;
...
******************************************************************************/
void QuickPID::SetTunings(float Kp, float Ki, float Kd, float POn = 1.0, float DOn = 0.0, uint32_t NewSampleTimeUs = 100000) {
  if (Kp < 0 || Ki < 0 || Kd < 0 || NewSampleTimeUs <0) return;
